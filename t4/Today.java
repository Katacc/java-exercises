...
public class Today {
    ...
    public static void main(String[] args) {
        ...
        if (args.length > 0) {
            ...
            try {
                MonthDay monthDay = MonthDay.parse(args[0], formatter);
                fullDate = monthDay.atYear(currentYear);

            } catch (DateTimeParseException e) {
                System.out.println("Please use the format: --MM-dd");
                return;
            }
            ...
            if (args.length > 1) {
                ...
                if (cat1.length == 2) {
                    selectionCat = new Category(cat1[0], cat1[1]);
                } else {
                    System.out.println("Make sure to use category1/category2 format");
                }
            } else {
                System.out.println("Make sure to include category!");
            }

        // print the events for given parameters
        app.report(fullDate, selectionCat);


        } else {
            System.out.println("Usage: Today <--MM-dd> <category1/category2>");
        }

    }

    // Modified report function to use parameters to print out events in "YYYY: Description" format
    private void report(LocalDate givenDate, Category givenCat) {
        // LocalDate today = LocalDate.now();
        for (Event event : this.events) {
            if (isSameDate(givenDate, event.getDate()) && (givenCat != null && givenCat.equals(event.getCategory()))) {
                String printableEvent = event.toPrintableEvent();
                System.out.println(printableEvent);
            }
        }
    }
